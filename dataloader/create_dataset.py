...
def load_pickle(path):
    with open(path, 'rb') as f:
        d = pickle.load(f)
        # [print((d[i][0][1].shape),(d[i][0][2].shape),d[i][0][4], d[i][0][5]) for i in range(100)]
        # exit()
        return d
...
class MOSI:
    def __init__(self, config):
        ...
        try:
            ...
            pickle_filename = '../datasets/MOSI/mosi_data_0421.pkl'
...
class IEMOCAP:
    def __init__(self, config):
        ...
        # If cached data if already exists
        # data = load_pickle(DATA_PATH + '/iemocap_data_0421.pkl')
        train_data = load_pickle(DATA_PATH + '/new_train_por_v4_0610_6c_sep_contexts_v.pkl')
        dev_data = load_pickle(DATA_PATH + '/new_dev_por_v4_0610_6c_sep_contexts_v.pkl')
        test_data = load_pickle(DATA_PATH + '/new_test_por_v4_0610_6c_sep_contexts_v.pkl')

        # self.train = data['train']
        # self.dev = data['valid']
        # self.test = data['test']
        self.train = np.array(train_data)
        self.dev = np.array(dev_data)
