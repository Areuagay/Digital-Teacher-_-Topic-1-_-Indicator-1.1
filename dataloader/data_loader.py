...
import random
from .tools import contain_nonum, is_number
...
class MSADataset(Dataset):
    ...
    def __len__(self):
        return self.len - self.len%self.config.batch_size
...
def get_loader(hp, config, prompt_dict = None, shuffle=True):
    ...
    if config.mode == 'train':
        hp.n_train = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'valid':
        hp.n_valid = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test':
        hp.n_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_mosi':
        hp.n_mosi_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_mosei':
        hp.n_mosei_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_meld':
        hp.n_meld_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_iemocap':
        hp.n_iemocap_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_laps':
        hp.n_laps_test = len(dataset) - len(dataset)%config.batch_size
    elif config.mode == 'test_res':
        hp.n_res_test = len(dataset) - len(dataset)%config.batch_size
...
def collate_fn(batch):
    ...
    encoding = tokenizer(
        ["sst2 sentence: " + sequence for sequence in inputs_seq],
        return_tensors="pt", padding=True
    )
    # T5 model things are batch_first
    t5_input_id = encoding.input_ids
