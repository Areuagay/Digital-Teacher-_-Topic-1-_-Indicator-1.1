...
from models.cross_attention.Transformer_block import EncoderLayer
...
class MultiModalCNN(nn.Module):
    def __init__(self, num_classes=8, fusion='ia', seq_length=15, pretr_ef='None', num_heads=1):
        ...
        self.video_residual_module = nn.Sequential(
            nn.Linear(64, 128),
            nn.LayerNorm(128),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
        ...
        self.audio_residual_module = nn.Sequential(
            nn.Linear(64, 128),
            nn.LayerNorm(128),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
        ...
        self.audio_feature1 = EncoderLayer(256, 512 , 256, 8, 32, 32,)

        self.video_feature1 = EncoderLayer(256, 512 , 256, 8, 32, 32,)

        self.audio_feature2 = EncoderLayer(256, 512 , 256, 8, 32, 32,)

        self.video_feature2 = EncoderLayer(256, 512 , 256, 8, 32, 32,)
        ...
        self.adim_reduction = nn.Sequential(
            nn.Linear(8192,256),
            nn.LayerNorm(256),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
        )
        self.vdim_reduction = nn.Sequential(
            nn.Linear(40192,256),
            nn.LayerNorm(256),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
        ...
        self.out_feature = nn.Sequential(
            ...
            nn.LayerNorm(self.text.bert.config.hidden_size),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
        )
    ...
    def forward(self, x_visual, x_audio, video_len, audio_len, inputs_text):
        ...
        x_visual = torch.transpose(x_visual, 0, 1)
        x_audio = torch.transpose(x_audio, 0, 1)

        x_audio = self.audio_residual_module(x_audio)
        x_visual = self.video_residual_module(x_visual)


        x_audio = x_audio + self.audio_feature1(x_audio)
        x_visual = x_visual + self.video_feature1(x_visual)

        x_audio = self.cross_attention1a(x_audio, x_visual) * (x_audio+1)
        x_visual = self.cross_attention1v(x_visual, x_audio) * (x_visual+1)
        ...
        audio_pooled2 = self.cross_attention2a(x_audio, x_visual) * (x_audio+1)
        video_pooled2 = self.cross_attention2v(x_visual, x_audio) * (x_visual+1)

        audio_pooled2 = torch.reshape(audio_pooled2,(16,8192))
        video_pooled2 = torch.reshape(video_pooled2,(16,40192))

        audio_pooled2 = self.adim_reduction(audio_pooled2)
        video_pooled2 = self.vdim_reduction(video_pooled2)
