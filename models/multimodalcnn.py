...
def conv1d_block(in_channels, out_channels, kernel_size=3, stride=1, padding='reflect'):
    return nn.Sequential(nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size,stride=stride, padding_mode=padding),nn.BatchNorm1d(out_channels),
...
def conv1d_block_audio(in_channels, out_channels, kernel_size=3, stride=1, padding_mode='same'):
    return nn.Sequential(nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size,stride=stride, padding_mode='reflect'),nn.BatchNorm1d(out_channels),
...
class MultiModalCNN(nn.Module):
    def __init__(self, num_classes=8, fusion='ia', seq_length=15, pretr_ef='None', num_heads=1):
        ...
        if fusion in ['lt', 'it']:
            ...
            self.av1 = AttentionBlock(in_dim_k=input_dim_video, in_dim_q=input_dim_audio, out_dim=e_dim, num_heads=num_heads)
            self.va1 = AttentionBlock(in_dim_k=input_dim_audio, in_dim_q=input_dim_video, out_dim=e_dim, num_heads=num_heads)

            input_dim_video1 = input_dim_video // 2
            self.av = Attention(in_dim_k=input_dim_video1, in_dim_q=input_dim_audio, out_dim=input_dim_audio, num_heads=num_heads)
            self.va = Attention(in_dim_k=input_dim_audio, in_dim_q=input_dim_video1, out_dim=input_dim_video, num_heads=num_heads)
       
        elif fusion in ['ita']:
            self.av1 = Attention(in_dim_k=input_dim_video, in_dim_q=input_dim_audio, out_dim=e_dim, num_heads=num_heads)
            self.va1 = Attention(in_dim_k=input_dim_audio, in_dim_q=input_dim_video, out_dim=e_dim, num_heads=num_heads)

            input_dim_video1 = input_dim_video // 2
            self.av = AttentionBlock(in_dim_k=input_dim_video1, in_dim_q=input_dim_audio, out_dim=input_dim_audio, num_heads=num_heads)
            self.va = AttentionBlock(in_dim_k=input_dim_audio, in_dim_q=input_dim_video1, out_dim=input_dim_video1, num_heads=num_heads)
    ...
    def forward(self, x_audio, x_visual):
        ...
        # print(x_audio.size(),x_visual.size())
        if self.fusion == 'lt':
        ...
        elif self.fusion == 'iat':
            return self.forward_feature_4(x_audio, x_visual)

        elif self.fusion == 'ita':
            return self.forward_feature_5(x_audio, x_visual)

        elif self.fusion == 'itt':
            return self.forward_feature_6(x_audio, x_visual)
    ...
    def forward_feature_3(self, x_audio, x_visual):
        ...
        # print(x_audio.shape,h_av.shape,x_visual.shape,h_va.shape)
    ...
    def forward_feature_4(self, x_audio, x_visual):
        ...
        # print(x_audio.shape,x_visual.shape)
        proj_x_a = x_audio.permute(0,2,1)
        proj_x_v = x_visual.permute(0,2,1)
    ...
    def forward_feature_5(self, x_audio, x_visual):
        x_audio = self.audio_model.forward_feature(x_audio)
        x_audio = self.audio_model.forward_stage1(x_audio)
        x_visual = self.visual_model.forward_features(x_visual)
        x_visual = self.visual_model.forward_stage1(x_visual)
    ...
    def forward_feature_6(self, x_audio, x_visual):
        x_audio = self.audio_model.forward_feature(x_audio)
        x_audio = self.audio_model.forward_stage1(x_audio)
        x_visual = self.visual_model.forward_features(x_visual)
        x_visual = self.visual_model.forward_stage1(x_visual)
